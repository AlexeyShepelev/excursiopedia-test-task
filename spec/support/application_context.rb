shared_context :application do
  ...
  def expect_data_in_table(selector, main_object, child_objects, child_name_method)
    ...
    child_objects[0...list_limit].each do |object|
      expect(table).to have_link(object.send(child_name_method), href: url_for([object, { only_path: true }]))
    end

    if children_count > list_limit
      child_objects[list_limit...children_count].each do |object|
        expect(table).to have_no_link(object.send(child_name_method), href: url_for([object, { only_path: true }]))
      end
    end
  end
